...
class PieceLogic {
    ...
    private fun rookVerticalPath(from: Square, to: Square): Boolean {
        ...
        for (i in 1..gap) {
            ...
            if (ChessGame.piecePosition(Square(from.col, nextRow)) !=null) {
                return false
            }
        }
        return true
    ...
    private fun rookHorizontalPath(from: Square, to: Square): Boolean {
        ...
        for (i in 1..gap) {
            ...
            if (ChessGame.piecePosition(Square(nextCol, from.row)) !=null) {
                return false
            }
        }
        return true
    ...
    private fun piecesBetween(fromCol: Int, fromRow: Int,
        ...
        if (fromCol == toCol) {
            ...
            for (row in start + 1 until end) {
                count += if (ChessGame.piecePosition(fromCol, row) == null) 0 else 1
            }
        } else {
            start = min(fromCol, toCol)
            end = max(fromCol, toCol)
            for (col in start + 1 until end) {
                count += if (ChessGame.piecePosition(col, fromRow) == null) 0 else 1
            }
        }
        return count
    }
    ...
    fun cannonLegal(from: Square, to: Square): Boolean {
        return if (ChessGame.piecePosition(to.col, to.row) == null) {
            rookLegal(from, to)
